...
plan: 01
subsystem: database
tags: [supabase, postgres, migration, trigger, jsonb, seed]

requires:
  - phase: 05-schema-evolution-content-seeding
    provides: wipe-in-FK-order + label-join seed pattern (migration 006)
  - phase: 12-v13-monday-prep-schema
    provides: idempotent DROP CONSTRAINT IF EXISTS + ADD CONSTRAINT pattern (migration 008)
provides:
...
affects: [14-02, 14-03, 15-hub-role-identity, 16-scorecard-weekly-choice, 17-admin-meeting-mode, 18-comparison]

tech-stack:
  added: []
  patterns:
    - Idempotent DDL (DROP CONSTRAINT IF EXISTS + ADD CONSTRAINT) carried forward from migration 008
    - Wipe-in-FK-order + seed-via-label-join carried forward from migration 006
    - Postgres BEFORE trigger with P0001 RAISE + message prefix as app-layer catch contract
    - Flat JSONB scalar values in admin_settings (no wrapper objects per D-12)
    - partner_scope='both' for shared mandatory templates (one row, fanned out via kpi_selections per D-04)
...
requirements-completed: [SCHEMA-01, SCHEMA-02, SCHEMA-03, SCHEMA-04, SCHEMA-05, SCHEMA-06, SCHEMA-07, SCHEMA-08, SCHEMA-09, SCHEMA-11]
