...
- **D-16 extension (2026-05-04):** Reflection text is now MANDATORY on every KPI row at submit time, regardless of result. Each KPI carries its own `reflection_prompt` (migration 015 added the column to `kpi_templates`); the prompt renders as helper text above the textarea. Pending rows still also require non-empty `pending_text` per KPI-02. Post-submit edits remain auto-saved without re-validation (per the Saturday-close edit window). Submit error: `submitErrorReflectionRequired` in SCORECARD_COPY.
