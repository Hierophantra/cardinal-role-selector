...
plan: 03
subsystem: hub-integration
tags: [partner-hub, role-identity, kpi-ready-gating, weekly-kpi-placeholder-route, no-approval-growth, trace-rename]
requires:
  - 15-01 (ROLE_IDENTITY shape, seasonStats rewrite)
  - 15-02 (RoleIdentitySection, ThisWeekKpisSection, PersonalGrowthSection prop contracts)
provides:
...
affects:
  - src/components/PartnerHub.jsx (full rewrite)
  - src/App.jsx (placeholder route + inline component)
  - src/components/Scorecard.jsx (1-line guard patch)
  - src/data/content.js (HUB_COPY.partner.cards.roleDefinition title + description)
tech_stack:
  added: []
  patterns:
    - "Static-data-first render (role identity renders before async fetches resolve)"
    - "kpiReady = kpiSelections.length > 0 gating (replaces kpiLocked/locked_until everywhere in hub + scorecard)"
    - "7-fetch Promise.all (role stats + weekly selection + growth priorities fanout)"
    - "Inner try/catch around post-save refetch so a blip does not invalidate a durable save"
    - "Hooks-before-early-return invariant (12 useState calls precede any conditional render)"
key_files:
  created: []
  modified:
    ...
    - src/data/content.js
decisions:
  - "Phase 15 cleanup scope for kpiLocked/locked_until = PartnerHub.jsx (full removal) + Scorecard.jsx (1-line guard) — Research §4; KpiSelection.jsx / KpiSelectionView.jsx intentionally deferred to later phases"
  - "Placeholder WeeklyKpiPlaceholder component lives inline in App.jsx rather than a new file — Phase 16 replaces it with the real WeeklyKpiSelectionFlow; no separate file churn"
  - "KPI_COPY import dropped from PartnerHub alongside the KPI Selection card deletion (M6 / N11) — confirmed zero remaining references"
  - "handleSaveSelfChosen wraps post-save fetchGrowthPriorities refetch in its own try/catch (N8) — refetch blip logs to console but does NOT surface as 'Couldn't save your priority'"
  - "Loading gate moved from early-return to conditional render — RoleIdentitySection renders immediately from static data, async children render after fetches resolve"
metrics:
  duration: ~4 minutes
