...
plan: 02
subsystem: hub-ui-components
tags: [role-identity, this-week-kpis, personal-growth, hub-redesign, no-framer, controlled-collapsibles]
requires:
...
affects:
  - src/components/RoleIdentitySection.jsx (new)
  - src/components/ThisWeekKpisSection.jsx (new)
  - src/components/PersonalGrowthSection.jsx (new)
  - src/index.css (append-only additions + 1 Rule-2 fix for missing .pending badge)
tech_stack:
  added: []
  patterns:
    - "Controlled collapsibles via CSS max-height transition (no Framer Motion, D-07)"
    - "Pure presentation components with hub-orchestrated data"
    - "Named-export helper function for unit-testable pure logic (statusModifierClass)"
    - "Local write-cycle state + prop-callback for persistence (PersonalGrowthSection)"
key_files:
  created:
  ...
  modified:
    - src/index.css
decisions:
  - "Prop contracts LOCKED — Wave 3 PartnerHub must conform exactly"
  - "PersonalGrowthSection does NOT receive `partner` prop (checker M5) — hub closes over partner in onSaveSelfChosen callback"
  - "Badge classes reused from existing `.growth-status-badge.active` (Locked) and new `.growth-status-badge.pending` added to Phase 15 block (Rule 2 fix — checker N7 assumed `.pending` existed but it didn't)"
  - "ThisWeekKpisSection CTA routes to `/weekly-kpi/:partner` — Wave 3 MUST register a placeholder route in App.jsx to avoid catch-all redirect"
  - "statusModifierClass exported as a named module export (checker B1) — allows direct unit-testing of status→class mapping"
  - "Collapsible expand/collapse state lives in parent (Wave 3 hub) per D-24 hooks-before-early-return rule"
metrics:
  duration: ~4 minutes
