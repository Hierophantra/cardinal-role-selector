...
plan: 03
subsystem: weekly-kpi-selection-flow
tags: [route, component, framer-motion, supabase, wave2]
dependency_graph:
  requires:
    - "16-01 (WEEKLY_KPI_COPY in src/data/content.js + weekly-selection CSS classes)"
    - "14 (fetchKpiTemplates, fetchWeeklyKpiSelection, fetchPreviousWeeklyKpiSelection, upsertWeeklyKpiSelection, BackToBackKpiError)"
    - "15 (getMondayOf, VALID_PARTNERS, PARTNER_DISPLAY, CATEGORY_LABELS, /hub/:partner route)"
  provides:
    - "Route-level /weekly-kpi/:partner 3-view flow: selection → confirmation → success"
    - "Commit-time lock behavior (returning user redirected to /hub/:partner once selection exists)"
    - "Back-to-back rejection surfaces inline via BackToBackKpiError instanceof check"
  affects:
    - "src/App.jsx (placeholder route swapped for real component)"
tech_stack:
  added: []
  patterns:
    - "AnimatePresence mode='wait' + 3 keyed motion.divs — matches KpiSelection.jsx pattern"
    - "Hooks-before-early-return discipline (Phase 15 P-U2)"
    - "getMondayOf() for week anchor — no toISOString drift (Pitfall 3)"
    - "instanceof BackToBackKpiError for typed-error discrimination (never pattern-match code/message)"
    - "baseline_action used as label_snapshot at commit time — immortalizes KPI text at time of commit"
key_files:
  created:
  ...
  modified:
    - "src/App.jsx"
decisions:
  - "Inline optional-pool filter (not useMemo) — filter is cheap, deps are simple, matches style elsewhere in file"
  - "Disabled previous-week card uses existing .kpi-card.capped class + new .weekly-kpi-disabled-label — reuses semantic of 'unavailable' rather than inventing new class"
  - "Error card on load failure mirrors KpiSelection.jsx shape (app-shell → container → screen) with Back-to-Hub ghost link — consistent recovery affordance"
  - "Selection view shows Back-to-Hub both at top (small ghost link) and in bottom nav-row — matches KpiSelection.jsx double-ghost pattern"
  - "Confirmation view wrapped in .scorecard-commit-gate class (existing from Phase 15 / Phase 16-01) — reuses commit-gate visual vocabulary"
metrics:
  duration: "~15 min"
