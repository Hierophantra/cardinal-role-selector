...
plan: 05
subsystem: weekly-kpi
tags: [counter, hub, debounce, locked-card]
requires:
  - 16-01 (WEEKLY_KPI_COPY.hubLockedLabel + hubLockedHeadingTemplate)
  - 16-01 (CSS: .kpi-counter-btn, .kpi-counter.has-count, .weekly-choice-locked-label)
  - 16-03 (incrementKpiCounter in supabase.js; weekly_kpi_selections.counter_value)
provides:
  - inline +1 counter pill UI on countable KPI rows (COUNT-01)
  - 500ms batched debounced writes with per-template delta accumulator (COUNT-02/03)
  - counter seed from weeklySelection.counter_value on mount (COUNT-04)
  - hub weekly-choice card post-commit locked state (D-03)
affects:
  - src/components/PartnerHub.jsx
  - src/components/ThisWeekKpisSection.jsx
  - src/lib/supabase.js (fetchKpiSelections select list)
tech-stack:
  added: []
  patterns:
    - per-template delta accumulator via useRef + batched setTimeout flush
    - optimistic local counter state + server reconciliation on seed
key-files:
  created: []
  modified:
    - src/components/ThisWeekKpisSection.jsx
    ...
    - src/lib/supabase.js
decisions:
  - Surface `countable` on kpi_templates join so ThisWeekKpisSection can gate pill render per row without an extra prop
  - `weeklyChoiceLocked` prop is plumbed for forward compat but not consumed in UI per D-03 (hasSelection alone drives the Locked label)
metrics:
  duration: ~12m
