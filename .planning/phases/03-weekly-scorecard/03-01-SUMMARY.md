...
tags: [supabase, postgres, react, vanilla-css, date-math, content-copy]

requires:
  - phase: 01-schema-hub
    provides: scorecards composite-PK table + kpi_results JSONB column
  - phase: 02-kpi-selection
    provides: kpi_selections.id UUIDs used as JSONB keys; lockKpiSelections gating
...
affects: [03-02-scorecard-ui, 03-03-partner-hub-integration, 04-admin-tools]

tech-stack:
  added: []
  patterns:
    - "Local-time week math (never UTC ISO slicing) centralized in src/lib/week.js"
    - "Initialize JSONB per-key shape on commit so React textareas stay controlled"
    - "Client-side derived week close (no cron) via isWeekClosed(mondayStr)"
    - "Content-copy separation extended with SCORECARD_COPY (parallel to HUB_COPY / KPI_COPY)"
    - "Additive SQL migration (add column if not exists) preserving Phase 1/2 semantics"
