...
plan: 03
subsystem: ui
tags: [react, scorecard, meeting-mode, kpi, reflection, weekly-rating]

# Dependency graph
requires:
  - phase: 06-partner-meeting-flow-updates
    plan: 01
    provides: SCORECARD_COPY reflection keys, MEETING_COPY kpiEyebrow(n, total), fetchKpiSelections with mandatory join
provides:
...
affects: [Scorecard.jsx, AdminMeetingSession.jsx]

# Tech tracking
tech-stack:
  added: []
  patterns:
    - "weekRating auto-save via useRef initialized guard + useEffect on weekRating change"
    - "allKpisAnswered useMemo gates Weekly Reflection section visibility"
    - "canSubmit = allAnsweredWithReflection && weeklyWin.trim() && weekRating !== null"
    - "KPI_STOP_COUNT = STOPS.filter(s => s.startsWith('kpi_')).length for dynamic total"
...
requirements-completed: [SCORE-06, SCORE-07, SCORE-08, MEET-05, MEET-06]
