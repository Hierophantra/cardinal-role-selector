...
plan: 02
subsystem: kpi-selection-ui
tags: [react, framer-motion, supabase, kpi-selection, phase-2-ui]
requires:
  - KPI_COPY constant in src/data/content.js (added in 02-01)
  - fetchGrowthPriorityTemplates / lockKpiSelections in src/lib/supabase.js (added in 02-01)
  - Phase 2 CSS block in src/index.css (added in 02-01)
  - VALID_PARTNERS / PARTNER_DISPLAY exports (pre-existing)
  - Framer Motion AnimatePresence pattern (from Questionnaire.jsx)
provides:
...
affects:
  - src/App.jsx (2 new imports, 2 new Route entries)
tech-stack:
  added: []
  patterns:
    - Framer Motion AnimatePresence view-swap pattern (view state as discriminator, shared motionProps object)
    - Partner-guard on mount with early-return pattern (matches PartnerHub.jsx)
    - Replace-all list persistence (delete-then-insert, by id, skipping locked rows)
    - Direct supabase client usage for growth_priorities row delete (no deleteGrowthPriority helper yet)
    - Per-slot state object `{ kind, templateId, customText }` for mutually-exclusive template-or-custom input
key-files:
  created:
  ...
  modified:
    - src/App.jsx
decisions:
  - View-switching is managed by a single `view` state variable ('selection' | 'confirmation' | 'success') rendered inside one AnimatePresence, rather than separate routes for each step — this keeps all in-progress state co-located without URL churn and satisfies D-05 / D-06 back-preserves-state contract
  - Existing non-locked rows are fully deleted then re-inserted on Continue, so the stored row set is always an exact mirror of the UI — no stale-row accumulation (Pattern 1 / Pitfall 2)
  - `supabase` client is imported directly into KpiSelection for the single growth_priorities row-by-id delete (no deleteGrowthPriority helper added — out of scope for this plan)
  - Growth priority slots store `{ kind, templateId, customText }` rather than a resolved description string, so round-tripping from existing DB rows back into UI state preserves whether the user picked a template or wrote custom text
metrics:
  duration: "~3m"
  tasks_completed: 3
  files_changed: 3
