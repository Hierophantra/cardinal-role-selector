...
plan: 01
subsystem: kpi-data-content-foundation
tags: [supabase, migration, content, css, phase-2-foundation]
requires:
  - 001_schema_phase1.sql (kpi_templates, kpi_selections, growth_priorities)
  - HUB_COPY.partner.status (existing)
  - src/index.css tokens (--bg/--surface/--red/--gold/--text/--muted/--border/--success)
  - @keyframes optionIn, selectPulse, fadeIn
provides:
...
affects:
  - src/lib/supabase.js (13 exports, was 11)
  - src/data/content.js (KPI_COPY added, HUB_COPY.partner.status extended)
  - src/index.css (Phase 2 block appended)
  - supabase/migrations/ (new file 002_kpi_seed.sql)
tech-stack:
  added: []
  patterns:
    - supabase throw-on-error async lib function pattern
    - HUB_COPY nested object extension (D-14 status line contract)
    - /* --- section --- */ labelled CSS block convention
key-files:
  created:
    - supabase/migrations/002_kpi_seed.sql
  modified:
    ...
    - src/index.css
decisions:
  - No strict unique index on growth_priorities(partner,type) - would break 1 personal + 2 business rule; upserts key on id instead
  - KPI template labels tagged "(placeholder)" so grep-style checks confirm placeholder status before partner meeting
  - HUB_COPY.partner.status.roleCompleteKpisLocked converted from string to (date) => string per UI-SPEC D-14 contract
  - lockKpiSelections returns ISO lock date so confirmation screen can render without recompute
metrics:
  duration: "~3m"
  tasks_completed: 4
  files_changed: 4
