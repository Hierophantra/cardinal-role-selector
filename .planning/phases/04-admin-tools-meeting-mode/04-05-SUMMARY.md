...
plan: 05
subsystem: routes-hub-wiring
tags: [routes, admin-hub, hero-card, phase-4, wave-3, integration]
requires:
  - src/components/admin/AdminKpi.jsx (from 04-02 — default export AdminKpi)
  - src/components/admin/AdminScorecards.jsx (from 04-03 — default export AdminScorecards)
  - src/components/admin/AdminMeeting.jsx (from 04-04 — default export AdminMeeting)
  - src/components/admin/AdminMeetingSession.jsx (from 04-04 — default export AdminMeetingSession)
  - src/data/content.js HUB_COPY.admin.cards (scorecardOversight added, disabledLabel removed in 04-01)
  - src/index.css .hub-card--hero (from 04-01)
provides:
  - /admin/kpi route
...
affects:
  - src/App.jsx (route registry extended)
  - src/components/admin/AdminHub.jsx (hero card + enabled accountability cards)
  - .planning/ROADMAP.md (Phase 4 marked complete 5/5)
tech-stack:
  added: []
  patterns:
    - "Hero hub card rendered outside .hub-grid as a standalone Link so it spans full width (Pitfall 6)"
    - "New admin routes registered after /admin/test and before the catch-all Navigate"
key-files:
  created: []
  modified:
    - src/App.jsx
    ...
    - .planning/ROADMAP.md
decisions:
  - "Meeting Mode lives ONLY in the hero position above the section grid — not duplicated inside the Accountability grid"
  - "Scorecard Oversight card uses clipboard icon \\u{1F4CB}; KPI Management keeps target \\u{1F3AF}; Meeting Mode hero keeps handshake \\u{1F91D}"
  - "Route order: specific /admin/meeting before /admin/meeting/:id (React Router v6 handles precedence either way, but this is clearer)"
  - "ROADMAP table row updated to '5/5 Complete 2026-04-11' in the same commit as the phase checkbox"
requirements:
  - ADMIN-01
  - ADMIN-04
  - MEET-01
