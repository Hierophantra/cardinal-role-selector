...
tags: [schema, supabase, copy, css, phase-4]
requires:
  - supabase/migrations/001_schema_phase1.sql (growth_priorities.status exists)
  - supabase/migrations/003_scorecard_phase3.sql (scorecards shape)
  - src/lib/supabase.js (assertResettable, fetchScorecard)
provides:
...
affects:
  - src/data/content.js HUB_COPY.admin.cards (disabledLabel removed from kpiManagement + meetingMode; scorecardOversight added)
tech-stack:
  added: []
  patterns:
    - "adminSwap updates kpi_selections by row id (no delete+insert)"
    - "upsertMeetingNote uses composite onConflict 'meeting_id,agenda_stop_key'"
    - "Render-time label fallback for kpi_results (no SQL backfill)"
key-files:
  created:
  ...
  modified:
    ...
    - src/index.css
decisions:
  - "Migration 005 does NOT re-add growth_priorities.status (already present in 001)"
  - "meeting_notes CHECK constraint enforces 10 fixed agenda_stop_key values (prevents typo note-loss)"
  - "adminSwapKpiTemplate intentionally omits locked_until so 90-day clock is preserved (D-05)"
  - "adminOverrideScorecardEntry snapshots label into kpi_results[kpiId].label per D-06"
  - "Phase 4 CSS spacing scale strictly 4-multiple — 14px/18px typography tokens upshifted to 15px/16px"
metrics:
  duration: "~4 minutes"
