...
plan: 03
subsystem: admin-growth-scorecard
tags: [admin, growth-priorities, scorecard-oversight, reopen, phase-4, wave-2]
requires:
  - src/lib/supabase.js (updateGrowthPriorityStatus, updateGrowthPriorityAdminNote, reopenScorecardWeek, fetchScorecards, fetchKpiSelections from 04-01)
  - src/data/content.js (GROWTH_STATUS_COPY, ADMIN_GROWTH_COPY, ADMIN_SCORECARD_COPY from 04-01)
  - src/index.css (.growth-status-badge, .growth-admin-note, .scorecard-oversight-*, .scorecard-reopened-badge, .meeting-admin-override-marker from 04-01)
provides:
...
affects:
  - None — route registration for /admin/scorecards deferred to P04-05
tech-stack:
  added: []
  patterns:
    - "Click-to-cycle status toggle (active -> achieved -> stalled -> deferred -> active)"
    - "Textarea onBlur save for admin notes (no debounce — single blur event)"
    - "Local isAdminClosed() wrapper over shared week.js isWeekClosed (Pattern 7 / Pitfall 5)"
    - "Render-time getLabelForEntry fallback for pre-Phase-4 kpi_results rows (Pattern 6 / D-06)"
    - "Two-click arm/confirm reopen with 3-second auto-disarm via useRef setTimeout"
    - "focusPartner query param reorders MANAGED array for deep-link UX"
key-files:
  created:
...
decisions:
  - "P04-03 scoped its AdminPartners edits to the growth priority subsection + scorecards deep link BELOW the kv block, well clear of the partner-card header nav-row where P04-02 added its 'Manage KPIs' link — parallel-wave merge safety"
  - "View Scorecard History is a Link button placed between the growth editor and Reset Controls, NOT in the nav-row that 04-02 owned, to avoid wave-2 merge contention"
  - "isAdminClosed() lives as a module-level helper in AdminScorecards.jsx only; src/lib/week.js isWeekClosed is unchanged because partner Scorecard.jsx also consumes it and must NOT respect admin_reopened_at (Pitfall 5)"
  - "getLabelForEntry() prefers entry.label (snapshotted by Phase 4 adminOverrideScorecardEntry) and falls back to kpi_selections.label_snapshot lookup — no SQL backfill needed (D-06)"
  - "Two-click reopen uses useRef setTimeout (3000ms) for auto-disarm, matching the existing AdminPartners ResetButton convention"
  - "Status cycle order matches D-09 / UI-SPEC 207-211: active -> achieved -> stalled -> deferred -> active (STATUS_CYCLE constant at module level for a single source of truth)"
metrics:
  duration: "~8 minutes"
